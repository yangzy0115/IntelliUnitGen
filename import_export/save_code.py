...
def set_global_paths():
    ...
    save_base_dir = filedialog.askdirectory(title=get_text('select_test_code_save_path'))
    if not save_base_dir:
        print("❌" + get_text('no_save_path'))
        sys.exit()

    run_base_dir = filedialog.askdirectory(title=get_text('select_project_run_path'))
    if not run_base_dir:
        print("❌" + get_text('no_run_path'))
        sys.exit()

    print(f"✅ {get_text('save_path')}: {save_base_dir}")
    print(f"✅ {get_text('compile_path')}: {run_base_dir}")
    return run_base_dir
...
def extract_java_code(statement_tests):
    ...
    if match:
        ...
        print("❗" + get_text('no_java_code_block'))
        return ""
...
"""
    ...
    try:
        with open(test_file_path, "w", encoding="utf-8") as f:
            ...
            f.write(java_code_cleaned)
        print(f"✅ {get_text('save_java_test_code_success')} {test_file_path}")
    except Exception as e:
        print(f"❌ {get_text('save_java_test_code_failed')}: {e}")
